...
- 理想低通特性：
  ...
  - 最高频带利用率为 $\eta=R_B/B=2$
- 余弦滚降特性：
  - 滚降系数 $\alpha=f_\Delta/f_N$ = 奈奎斯特带宽 / 超出奈奎斯特带宽的扩展量
...
- 2ASK (二进制振幅键控; OOK)：1 通 0 不通
  ...
  - 解调
    - 包络检波：带通滤波 - 全波整流 - 低通滤波 - 抽样判决
      - 误码率 $\displaystyle P_e = \frac12e^{-r/4}$，判决 Vd\* = 0
    - 相干解调：带通滤波 - 相乘器 - 低通滤波 - 抽样判决
      - 等概最佳判决门限 a/2
      - 误码率 $P_{e}=\frac{1}{2} erfc(\sqrt{\frac{r}{4}})$，输入信噪比 $\displaystyle r=\frac{a^2}{2\sigma_n^2}$，大信噪比时 $\displaystyle P_e\approx\frac1{\sqrt{\pi r}}e^{-\mathrm{r}/4}$
    - 相干解调无门限效应，信噪比不大时抗噪性能更好，但设备复杂
- 2FSK（二进制频移键控）：双载波，1 时用 f1，2 时用 f2
  - 带宽 $B_{2\mathrm{FSK}}=\begin{vmatrix}f_2-f_1\end{vmatrix}+2f_s$
...
- A 律 13 折线：8 位折叠码，首位表示极性，2-4 段落码，5-8 段内码
