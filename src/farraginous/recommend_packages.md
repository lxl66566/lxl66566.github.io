...
- 如果某些软件没有说明文字，看看章节标题有没有颜色，如果是链接可以点进去看看。
...
> 不过不得不提，mpv 的文档是**垃圾**的水平。我想实现的 [globalspeed](#global-speed) 里提供的功能，直接用 conf 是写不出来的，写 user script 又没有文档查。。。

### [RAM Disk](../articles/ramdisk.md)
...
在工作或游戏时，经常休息一下眼睛是有必要的。这里做一个简单评价。

- Windows：
  - [eyesbreaker](https://github.com/lxl66566/eyesbreaker)：我写的一个非常简单的托盘图标，每 30 分钟会亮起来，提醒去放松眼睛。问题就是不太明显，可能会忽略；而且用了几个月经常看到亮起来就鼠标顺手划过去了，起不到提醒护眼的效果。
  - [RustClock](https://github.com/hoothin/RustClock)：更近一步，每 30 分钟会从侧面弹出一个时钟。虽然代码比较屎，作者已经不更新了；更大的问题是弹出会打断当前输入法内容和窗口焦点。
  - [ProjectEye](https://github.com/Planshit/ProjectEye)：更进一步，强制全屏打断你提醒护眼。对于自制力不高的人，这种强制打断比较有用。可以配置的东西也比较多，跟 Tai 一个作者，Planshit 还是可以的，不过现在也不更新了。
...
- [Obtainium](https://github.com/ImranR98/Obtainium)：可以从 Github release 里安装 apk 的包管理器。和我的 [bin-package-manager](https://github.com/lxl66566/bpm) 有异曲同工之妙，不过它做得更好看实用。
  - 而且 `.apk` 确实非常适合这样下载。我的 bin-package-manager 要考虑的就多了。
  - Obtainium 的 Github 搜索做得比较差，可能遇到仓库名完全匹配但是搜不到的情况。
...
### [浏览器](../articles/browser/assess.md)
...
### [音乐播放器](../articles/android_player.md)
...
1. 请使用[代理](../articles/proxy/index.md)以获得更好的 scoop 使用体验：`scoop config proxy 127.0.0.1:<port>`，并且还需要配置 [git 走代理](../coding/Git.md#配置)。
2. 如果你的电脑没有禁用 Windows Defender，请务必将 scoop 安装目录（默认为 `~/scoop`）添加到 Windows Defender - Virus Protection 的排除列表 ([ref](https://github.com/ScoopInstaller/Scoop/issues/4491))。
...
scoop update <name>               # 更新包，* 表示全部。scoop 每次 update 默认会保留旧版本程序。
scoop reset <app>@<version>       # 回滚到之前的版本
```
...
- 安装 scoop 和 git 后，也可以 `scoop install aria2c` 以进行多线程下载。
...
- [7-Zip-zstd](https://github.com/mcmilk/7-Zip-zstd)：添加了 zstd 支持，优化了 GUI 界面。我正在用这个。
...
有时候我们希望让电脑做一些机械化自动化的操作（例如游戏挂机等）。AHK 可以写脚本执行复杂逻辑，但是鼠标的定位与坐标换算也是比较麻烦的。

- [tinytask](https://tinytask.net/) 是一个轻量的操作录制与回放软件，可以复刻一些比较基础的操作。
  - 使用时注意一定要提前记好 playback hotkey，避免 playback 时无法停下的尴尬场面。
...
开源沙盒，可以进入隔离环境，有时候用得到。请务必使用沙盒运行未知来源的程序。

- 使用 sandboxie 打开资源管理器，写入的任何文件和本机都是隔离的，包括 RAM Disk 也是隔离的。使用时务必注意你的窗口是否有黄色的边框，免得认错环境。
  - sandboxie 居然实现了 CoW，有点先进。
- sandboxie 内没法访问 msys2 和 git，这在 AI 编程时代是一个绝大败笔。
- sandboxie 内打开的终端没法粘贴（剪贴板跟外部不同步）。
...
- 注意：在笔记本上（特别是独显 + 核显混合的笔记本上）使用 TrafficMonitor，硬件监控里一定不要启用显卡监控。否则在独显进入休眠的时候会被 TrafficMonitor 戳一下唤醒，然后再进入休眠再唤醒，最终导致 System 进程高占用，[很难排查](https://superuser.com/questions/1669541/high-system-cpu-and-sluggish-response-after-connecting-egpu)。
...
这里是一些主要给开发者/geek 使用的妙妙小工具。当然如果你喜欢折腾也可以尝试一下。

## [Process Monitor](https://learn.microsoft.com/en-us/sysinternals/downloads/procmon)

监控进程事件，软件行为分析的瑞士军刀。可以查看软件读写的文件/注册表，加载的 dll 等。

## [DebugView++](https://github.com/CobaltFusion/DebugViewPP)

用来看系统的 debug log，比微软官方的 DebugView 好用很多，复制日志也方便。
...
这个软件不只是开发者在用，游戏玩家也广泛使用。

- 绑核：大小核架构上把游戏进程绑到大核；或者 X3D CPU 上把游戏进程绑到有大 L3 cache 的 CCD，以发挥 CPU 优势提高游戏帧率。
- 永久设置/取消设置效率模式，用来[解决一些 bug](https://t.me/withabsolutex/2656)。

## [TaskExplorer](https://github.com/DavidXanatos/TaskExplorer)

比任务管理器更强大，可以看到更详细的进程信息。但是不要一直开在后台，否则会一直给 Kernel 发查询，打断 Kernel 休眠，损失性能。

## Linux

前往 [Linux - 包管理与使用推荐](../articles/linux/package.md#包使用)
...
### [划词高亮页面内相同文字（效果类似划词自动 ctrl + f 在页面上搜索）](https://gist.github.com/lxl66566/c314e5eeb03f4f2504bd1c09561a12cc)
...
视频下载**命令行**工具。但是现在随着视频网站权限收紧，yt-dlp 变得不太好用（下 B 站的还行，youtube 就很麻烦了），而且 AI 时代也有一些 vibe 的视频下载器，不一定要用这个。

### [EnergyStarX](https://github.com/JasonWei512/EnergyStarX)

[EnergyStar](#energystar) 有人接手，这个做的还不错，有 GUI，并且能自动在不插电时启用。

但是这个神人玩意只给 msix 不给 portable，msix 在我的机器上也跑不了。蒜鸟蒜鸟。
...
- 该应用在我的手机上经常自启，因此建议不用时冻结。
- 目前我使用 [reader](https://github.com/hectorqin/reader) 看书，这个软件只作为离线的备份使用。
- 更多使用习惯与设置，参考 [读书](../hobbies/books.md) 页面。
