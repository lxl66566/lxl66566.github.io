...
这两个方式具体哪个更好，一直有人持不同意见。目前看社区好像第一种方式比较占上风，因为 niri-flake 写得确实烂。具体要使用哪种，我的观点是：如果你需要在不同的电脑上使用不同的 niri 配置，或者有一些模块化配置具有侵入性，需要修改 niri 的部分配置，那就用 niri-flake，可以借用 nixos 的配置合并等机制。否则就用 kdl 吧。
