...
- 碰到 `HTTP error 200 (curl error: Stream error in the HTTP/2 framing layer)` 不要管，后台会重试。
...
还有一个学习方法是多看别人的 configuration，~~并且大量摘抄~~。我的配置在[下面](#配置)，还有一些：[ryan4yin's config](https://github.com/ryan4yin/nix-config) [nyancat](https://git.ny4.dev/nyancat/flake/src/branch/master) [wimpysworld](https://github.com/wimpysworld/nix-config) [jackdbd](https://github.com/jackdbd/nix-config) [nmasur](https://github.com/nmasur/dotfiles) [oo-infty](https://github.com/oo-infty/nixos-configurations) [AsterisMono](https://github.com/AsterisMono/flake) [tama-gucci(dendritic pattern)](https://github.com/tama-gucci/.dotfiles) [lovesegfault](https://github.com/lovesegfault/nix-config)
...
- NixOS module 里有一些 top level attribute，例如 `imports`, `options`, `config`，还有 `disabledModules`。其他非 top level 的项即使写在顶层，eval 时默认也会被包在 `config = {...}` 里面。
...
sudo nix-env --list-generations --profile /nix/var/nix/profiles/system            # 列出所有 NixOS 的历史版本和 id
sudo /nix/var/nix/profiles/system-<id>-link/bin/switch-to-configuration switch    # 重置整个系统到某个 id 的版本
sudo nix-env --profile /nix/var/nix/profiles/system --switch-generation <id>      # （可选）：让下次重启也使用某个 id 的版本
...
- ~~<https://home-manager-options.extranix.com/>：查找 home-manager 中的设置项~~（2026 年 nixos 官方的 option search 已经支持同时查找 home manager 的设置项）
- <https://status.nixos.org/>：查看 nixpkgs 各个 channel 的测试状态。如果你喜欢追新，或者打算要 update flake 可以看看。按照版本从新到旧排序应该是 master > nixos-unstable-small > nixos-unstable > nixos-stable-small > nixos-stable-small。
- ~~<https://noogle.dev/>：nix 语言学习查找~~（其实很多定义都缺了，很捞）
- <https://nur.nix-community.org/>：NUR 包
- [Nix 落絮](https://luoxu.torus.icu/)，搜群友聊天记录
...
[我的配置仓库](https://github.com/lxl66566/nixos-config) (temporary down)

202507 回坑 NixOS 时，我将整个配置重新拆成类似 Rust feature gate 的形式，符合我的 [tag 论](../../gossip/va_view.md#tag-论)。这样我就可以在各种系统上方便地组合 feature，以定制最符合我的需求的配置。

同时我的配置里也有一堆模块，每个模块是一个或多个软件及其配置组成的单个功能的集合。这样对于这部分功能可以方便拆装，而不需要频繁拆装的则直接扔到 feature gate 相关的模块里。

感觉上，其实有点借鉴 flake 的思想，stable 的就扔 nixpkgs，需要灵活组合的就扔 flake。（当然现在的 flake 已经丧失“灵活”代名词的地位了）。
...
- [den](https://github.com/denful/den)：还没细看，README 看起来跟我的理念挺符合的，复杂度也有点高。
...
substituters 可以理解为 nixos 的 binary cache，如果命中的话就可以直接联网下载构建产物而不是自己从源码编译了。

高校镜像站的 binary cache 从 cache.nixos.org 直接同步，并原封不动保留官方的签名，因此直接将镜像站地址添加到 substituters 里即可。有一些第三方（cachix、garnix）的 binary cache 是构建机自己构建的，签名和官方不一致，这时候就还需要额外将提供者的公钥加入 trusted-public-keys 中才允许拉取。

设置 substituters 有个问题是 bootstrap，如果你是在 nix configuration 的 `nix.settings` 里设置的 substituters，则首次 build 是无法使用的，因为该配置还没应用到系统中，因此首次构建拉不到 binary cache，如果想引入啥包，需要拆成两次构建。解决这个问题的好办法是直接把 substituters 写在 `flake.nix` 的顶层 `nixConfig` 中，这样本次构建可以直接应用，不需要 bootstrap。
...
图形桌面有两大类，Window Manager (WM) 和 Desktop Environment (DE)，前者比较底层一些，更难上手，不过自由度也更高。后者就跟 Windows 那种手感差不多，学习成本低。

我最开始是 KDE，现在已经转到 niri 了。

::: tabs

@tab niri

niri 是一个基于 Wayland 的平铺式 WM，使用 Rust 编写。

NixOS 的哲学是可复现，而 KDE 家的东西可复现性实在是太差了，然后我也想尝试一下新东西，所以用了 niri。我首次尝试 niri 的时候只是尝鲜心态，当时还没有 xwayland-satellite 这玩意，Wayland 基本还是一坨大便，所以没多久就弃坑回到 KDE 了。后来生态好起来了，才又慢慢迁移到 niri。
...
- xwayland-satellite，这个必装，一个易于使用的兼容层，让 niri 可以运行 x11 软件。niri-flake 中有选项可以开启。
...
- 剪切板也需要自己搞。直接上配置：
  ...
  }:
  {
    environment.systemPackages = with pkgs; [
      wl-clipboard
      # wl-clipboard-rs # do not use this because https://github.com/YaLTeR/wl-clipboard-rs/issues/5
    ];
...
我一直不喜欢 Gnome，之前用 Arch 的时候一直都用的 KDE。因此初入 NixOS 也先使用了 KDE。

具体的配置与推荐可以看 [Arch 那边的文章](./install_and_config.md#kde-及其配套设施)。
...
在原生 NixOS 上跑代理，我没遇到过问题。真正棘手的是在 NixOS-WSL 上跑代理：dae 无法使用，因为 WSL 内核默认不支持 ebpf；而用 v2rayA 又很丑且支持的协议不多，我希望能使用 Windows 下的代理软件。然后就遇到了无穷多的问题：

问题一：即使设置了 `HTTP_PROXY` 等环境变量，在 NixOS 去 fetch github 时也无法生效。因为此时实际上去 fetch 的是 nix-daemon，而这玩意是不吃终端的代理的。解决方法是修改 nix-daemon service，有一个临时方案和一个永久方案。
...
下面的方法也可以，但是上面更简洁，并且上面也会同时给终端环境设置 proxy env。（会设置成小写的 http_proxy 等）
...
systemd.services.nix-daemon = {
  ...
  preStart = ''
    ...
    if [ -n "$GATEWAY_IP" ]; then
      echo "http_proxy=http://$GATEWAY_IP:10450" > /run/nix-daemon-proxy.env
      echo "https_proxy=http://$GATEWAY_IP:10450" >> /run/nix-daemon-proxy.env
      echo "all_proxy=http://$GATEWAY_IP:10450" >> /run/nix-daemon-proxy.env
      echo "no_proxy=${noProxy}" >> /run/nix-daemon-proxy.env
    fi
  '';

  # 让 nix-daemon 加载这个动态生成的环境变量文件
  # 前面的 "-" 表示如果文件不存在也不要报错
