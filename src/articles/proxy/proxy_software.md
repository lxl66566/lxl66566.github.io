...
我的选择：我在 Windows 上使用 V2rayN，在 Linux 上使用 dae，在 Android 上使用 NekoBox。

虽然我也很想统一，很想只维护一套配置，但是在各种各样的权衡之下，也只能形成现在这种局面。

我还做了一个[实验性质的 dae <-> sing-box 的配置转换器](https://github.com/lxl66566/sing-dae)，以尽量避免维护两套配置的繁琐程度。

## 基础概念：内核与前端

- 内核：拦截/加密/封装流量，与远端节点通信。
- 前端：给用户操作，写配置。

一般代理都是这种架构。

下面文章是站在用户使用角度进行评价，实际上也只是按照前端划分了一下大致派系。有一些前端例如 v2rayN 支持一大堆内核，但是也被划分到 v2ray 系，希望读者不要过度纠结。
...
- [神秘模块](https://tutor.xireiki.com/)：magisk module
...
相比于 clash，v2ray 系列要灵活得多。V2rayN 等前端不仅支持 clash 格式的 yaml，也支持自定义节点配置，功能更加丰富。
...
_设置 - 路由设置_ 里可以编写自己的分流规则。一些注意事项：

- 常用的规则前缀有 `domain:`，`geosite:`，`geoip:` 等。
- `domain:` 开头的是 domain_suffix 匹配，可以匹配子域名。
- 如果不写 `domain:` 等前缀，默认是 keyword 匹配。
...
v2rayA 是一个跨多平台的、基于网页的代理前端。它有一个易于使用的网页面板，对于初入 linux 的用户比较友好；它也支持透明代理、分流规则、订阅链接等。

但是 v2rayA 只支持 v2ray 内核而不支持 Xray/sing-box 等现代内核，因此对协议的支持非常有限。
...
我大概在 2021 年以前使用过一段时间 V2rayNG，感觉有非常多的 bug，例如启动代理会越来越慢。
...
dae 是一个基于 eBPF 的代理内核，性能高。这是我目前用过的相对舒服的代理软件，可以**维护多个节点池**，对于每个池中的节点，都可以自动选择最小延迟的节点进行使用，并能根据不同规则进行节点池分流。缺点是只能用于 linux 系统，并且支持的协议比起其他代理软件偏少。

dae 是命令行与配置文件的，而 daed 是基于 dae 的 web 前端。如果你刚刚接触 dae 系代理或者不喜欢写配置文件，可以使用 daed。否则我还是比较推荐 dae 的。
...
1. [dae 与其他客户端的性能 benchmark](https://docs.google.com/spreadsheets/d/1UaWU6nNho7edBNjNqC8dfGXLlW0-cm84MM7sH6Gp7UE/edit?pli=1&gid=1965963111#gid=1965963111)。可以看出 dae 的直连性能远超其他实现。
