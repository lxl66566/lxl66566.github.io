...
- 软链接不是 _快捷方式_。快捷方式本质上是一个 `.lnk` 文件，与真正的软链接不同。
- 不能在 wsl 内使用 `ln -s` 创建。这样创建的软链接不会被 windows 识别。
- [msys2 的一个 PR](https://github.com/msys2/msys2-runtime/pull/114) 讨论了为什么 Windows 不允许非管理员创建软链接、以及如何允许这个行为并与 msys2 集成。
...
例如，在 Windows 上跑雷电等 Android 模拟器可以看到提示：_虚拟服务未关闭，会导致游戏异常或卡顿，建议立即修复！_。跑 Xmrig 等极致性能要求的程序，也容易被虚拟机的上述问题给坑到，导致 Hashrate 受到影响。

此外还有网卡驱动占用大量 CPU、还帧吞吐压力大了会掉驱动[^ppproblem]等问题。

[^ppproblem]: [ref](https://t.me/supersocksr_group/62746)

如果你不使用 WSL，请关闭虚拟化：`bcdedit /set hypervisorlaunchtype off`，随后重启。
