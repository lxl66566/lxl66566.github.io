    ...
    with open(file_path, "rb") as f:
        ...
        if magic != b"YPF\x00":
            if magic == b"MZ\x90\x00":
                ...
                print("这是一个 EXE 文件，需要查找内部的 YPF 数据")
                return None
            print("不是有效的 YPF 文件")
            return None
        ...
        print(f"猜测到的 YPF 密钥：0x{key:02X}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(f"用法：python {sys.argv[0]} <ypf 文件路径>")
        sys.exit(1)
...
后来我又[装了个日版 windows](./windows/settings.md#日版-windows-10)，尝试了一下最后的希望，也就是用 YSPac 封包。结果 <http://yu-ris.net/> 的都测过一遍，要么 gal 崩溃要么无声。打开 ypf 版本一看，YU-RIS β4.8 2025/04/30 打出来的 ypf 版本还是 492，还没有本游戏的 500 高。而这游戏已经是三年前的了。属实是活全家引擎。
...
这个 SiglusExtract 解出来的 ogg 格式是非标的，至少有 `Symphonia error: decoder needs to be reset`。然后定睛一看，这工具根本没有 pack 音频的选项啊。

我承认这个作者很强，甚至开发了 siglus_rs 这种 SiglusEngine 模拟器。但是这个 SiglusExtract 并不好用也是真的。
...
至于封包……[Dir-A 的这篇文章](https://www.cnblogs.com/Dir-A/p/18096904)说 EntisGLS 是「支持少量修改或直接支持免封包处理功能的引擎」，然而我自己（去除 `voice.noa` 后）用 procmon 扫了一下，也没有读 `voice.noa` 以外的相关语音文件，感觉还是需要封包的。

noa 是封包格式，封包内部的音频是 mio 格式（私有编码格式）。那么现在对 EntisGLS 加速就需要完成 4 步：**解 noa，解 mio，封 mio，封 noa**。

- `voice.noa` 可以 GARbro 直解，内部是 mio 音频格式，GARbro 可以帮忙转成 wav。前两步都解决了。
- 关于封 mio，从 EntisGLS 引擎里可以找到一个 `Cotopha/bin/wav2mio.exe`。随便探索一下用法，发现 `wav2mio xxx.wav` 就能转。所以第三步也解决了。
  - 但是如果使用 `wav2mio` 处理加速后的 wav 会报错 `PCM 形式の wav ファイルではありません。`。试了一下，`wav2mio` 的输入 wav 必须是 16bit 单声道的 wav，因此要把音频再刷一遍：`fd -e wav -x ffmpeg -i {} -acodec pcm_s16le -ar 16000 -ac 1 -bitexact -y {.}_temp.wav` 然后再重命名。
- 剩下第四步有点困难，GARbro 不支持封包，只能寄希望于官方的 `noa32c` 了。尝试了各种方式，`./noa32c.exe /p /raw voice voice.noa`、是否使用 `/l list.txt`、是否使用 `/d`，是否在 voice 文件夹内部开始打包，都会创建一个空的 `voice.noa`。在 bash 里使用 `../noa32c.exe /p /raw *.mio ../voice.noa` 会报错 `bash: ../noa32c.exe: Argument list too long`。根据[这个帖子](https://forums.fuwanovel.moe/topic/22068-help-regarding-noa-archives/)死马当活马医，去用 cmd 在 voice 文件夹内部执行 `noa32c.exe /p /d /raw *.mio out.noa`，然后就成了。。放到游戏里能跑。。。

探索过程中的其他发现：

- EntisGLS 引擎里的 exe 都必须运行在 Shift-JIS 下，执行命令前需要先 `chcp 932`，否则输出是乱码。
- 官方的 `noa32c.exe`（乃亞アーカイバ version 1.10A）也不能解缘之空的 `voice.noa`：`voice\AK000001.mio ファイルを開けませんでした。` 给我无语住了。
...
- [crskycode/GARbro](https://github.com/crskycode/GARbro)：最流行的一个 GARbro fork，但是 2025 年作者删库删帐号了。（2026.04 又打赢了复活赛）
...
def callback(in_data, frame_count, time_info, status):
    ...
    # 使用 librosa 降低一个八度（-12 个半音）
    shifted_audio_float = librosa.effects.pitch_shift(
        y=audio_float,
        sr=RATE,
        n_steps=-12,  # 负数表示降低音调，-12 个半音即一个八度
    )
    # f32 转回 int16
...
于是将目光投向 SoundTouch：如果我直接拿到 buffer 数据然后用 ST 对原始数据进行不变调加速呢？WSOLA 的效果还是相当值得信任的，毕竟天天解封包然后 ffmpeg 加速也是用的 WSOLA（[严格来说不是](../coding/audio.md)），质量有保证。

在更早的时候，dll wrapper 的可行性刚刚确定，我就已经有了这个想法。当时尝试 vibe 了一个使用 SoundTouch 直接对音频数据进行加速的 dll wrapper，可惜的是其无法正常使用，我就暂时搁置了这个想法。直到后来 V0 已经相对成熟，恰逢 Gemini 出了 3 pro，我便乘着 LLM 升级的西风开始了 V1 的迁移工作。
...
- 区分人声和语音：
  ...
  - 后来直接通过[音频处理里的人声特征判定](../coding/audio.md#人声判定)去做了。
    - 初级频域分析的效果很差，我决定直接做纯时域分析，包含时长、连续高能量帧、低能量帧比率、动态范围等。
- 如何判断同一语音：每次 Play 判定语音并入队。一般 buffer 可以被 Lock 很多次，但是 Play 一般每段语音只有一次。

经过一些测试，可以在 _ジュエリー・ハーツ・アカデミア_ 上正常运行了……

が、事情真的有这么简单吗？当我将逻辑搬到 Artemis 上就出了问题。

- 最大的问题其实是 APP 给音频的逻辑，给出的一些语音会在中间插入一段较长的静音，这点也是我 dump WAV 才发现的。我尝试了很多方法都无法解决这个问题，只好在入队时多做一次去 0 的处理。

然后在 Artemis 上跑通以后，Krkr 又出了问题，而且一个比一个棘手。。。这时候我已经隐约感觉到了，搞语音不中断其实比加速还难。

- krkr 里所有语音、音效都是 0.5s 一个 buffer。如果按照当前做法，把 0.5s 判为 SE 则没有任何效果；把 0.5s 判为语音则每句话只说前 0.5s，并且由于每次点击/鼠标进入按钮/移出按钮都会触发 SE（即使已经把 SE 设为静音），会导致语音被 SE 抢占，延后几秒钟。
...
1. 对于 krkr 的流式播放问题，采用 stream ctx + 队列进行流控，封装一个播放器进行音频播放；拦截游戏的停止播放事件，给流标上 End of stream 标记，最终停止行为由播放器控制。
2. 提高人声识别准确率，这里使用了一些更极致的做法，直接上模型，[silero-vad](https://github.com/snakers4/silero-vad) + onnxruntime 来识别人声。
