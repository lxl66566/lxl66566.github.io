...
观察日志可以了解到一些 caddy 的机制：

- 重复 `"msg":"replaced certificate in cache", "subjects":["*"], "new_expiration":...`：caddy 有一个有效期为 12h 的内部证书。每当证书剩余有效期不足 1/3（对于内部证书大约为 3.8 个小时）时，就会自动触发证书续期。所以这个操作相对频繁，也会在日志里留下较多的内容。
