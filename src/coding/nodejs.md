...
（截至 2025 年）当前各个包管理器竞争都很激烈，yarn 可以解析 npm 的 lockfile，而 pnpm 更狠，可以解析 yarn + npm 的 lockfile。对于老项目，可以放心大胆转成新的包管理器，而不用担心依赖版本问题。

::: tabs

@tab pnpm

[pnpm](#pnpm) 同时使用软链接和硬链接，一块硬盘上同一个依赖不会被复制多次，在节省空间上具有很大优势，我非常喜欢。安装速度也非常快。

在 2023 年以前，普及度还是略逊一筹（可能难以找到非常规问题解法），并且老项目兼容性会差一点。但是到了 2025 年，在高版本 nodejs 上，兼容性已经接近 100%，完全没有问题。

pnpm 的一个槽点是默认会修改 package.json，添加一个 `packageManager` 字段，这样会阻止其他包管理器对该项目的依赖操作。是有一些霸道的。
...
- yarn link 的软链接并不是对包进行软链接，而是用于在本地开发时将一个包链接到另一个项目中，从而无需发布包即可进行调试。反正挺鸡肋的。
- 我甚至找不到只更新 lockfile，但是不动我 node_modules 的方法。
