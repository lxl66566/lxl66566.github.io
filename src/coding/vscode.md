...
这个功能正常来说用不到，但是只要上班了就知道有多好用。。

公司可能强制要求安装一些编辑器插件，用于提交签名等。但是我不希望这个插件在我的其他项目中激活，避免扫描到一些不好的东西，这时候就可以用 vscode 的工作区机制隔离工作环境，让该插件只能在工作环境使用。

左下角点击 _设置 - 配置文件_，按照提示操作即可。
