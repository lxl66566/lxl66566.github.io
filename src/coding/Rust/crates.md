...
其次，concurrent-queue 引入了一个 [full_fence 的 lock not hack](https://github.com/smol-rs/concurrent-queue/blob/f41c20d8b9424eff6c227dead7b062e3b87142bc/src/lib.rs#L630-L660)，但是这个 hack 在 2026 年已经过时了。实测 LLVM 已把 x86 的 fence(SeqCst) 降级为单条 `lock orl $0x0,-0x40(%rsp)`，做了这个 hack 反而有两条指令 `movq   $0x0,(%rsp)` + `lock notq (%r15)`，导致性能退化 ~8-10%。
