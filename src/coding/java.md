...
- 对于中大型项目，安装微软的 _Debugger for Java_。然后写一个 `.vscode/launch.json`，具体内容可以从 idea 导出 XML 配置后，再让 AI 生成。然后就可以在左侧边栏找到 debugger 图标，运行或调试了。这里是我的 example。
  ...
  ```
  - _Debugger for Java_ 还有一个非常好用的地方是可以 Hot Code Replace，也就是调试过程中本地修改的代码可以直接生效，无需重启后端。这对于一个光是启动就要两分钟的 java 项目来说，是非常节省时间的。
...
> 被恶心了十几天以后，我最终还是选择了 vscode。vscode 虽然比较弱鸡，但它不会恶心我。

idea 有社区版，这个是免费的，不需要破解，理论上也够用，没必要给 jb 送钱。
...
写后端，有一个 API Client 几乎是必须的。它的基础功能是导入从浏览器复制的 curl，然后你可以修改一些参数或者 header 再发出请求，进行一些功能验证。

推荐使用 bruno，postman 的开源替代。postman 吃相太难看了。

## 其他注意事项

- vscode 按住 ctrl 跳转，跳到了基类实现。但是实际的实现根据传入的对象，可能是基类实现也可能是子类实现。所以开启一下 implementation lens 是比较重要的。不过话又说回来，这个 lens 的加载巨慢，因此可能也是没什么用的。
  ```json
  {
    "java.implementationCodeLens": "methods",
    "java.referencesCodeLens.enabled": true
...
- java 的 if 语句中只能放入 boolean 或其包装类。
...
Stream.of(1,2,3).anyMatch(x -> x == null);                                      // 任一元素满足。使用 allMatch 是所有元素满足。
```
...
- 不要定义 `get`、`is` 开头的非 getter 函数！否则会当成 getter 进行序列化，最终结果和预期不同。（之前不写 java 的很容易被坑到）
...
- DTO（Data transfer object）、DO（data object）都相当于 python dataclass，二者的区别是用途：DO 一般是进 DB 的，DTO 是跟其他服务交互的。
...
- 注意：swagger 生成的类型并不完全正确！！因为生成时依赖 Bean Getter，但是序列化和反序列化时又是依赖 Jackson 或者其他逻辑。
  - 因此最好不要在 DTO 上使用 `org.apache.commons.lang3.tuple.Pair`，因为 swagger 不能正确生成它的类型。
