...
- 如何查看某段时间前的提交记录？可以用类似 `https://github.com/NixOS/nixpkgs/commits/nixos-unstable@{7day}` 的这种方法。

### 其他搜索工具

- [grep app](https://grep.app/)，号称是最快的 Github 搜索工具，用来搜源码。但是问题是它不保证一定能搜到已存在代码。
- [Githubhunt](https://githubhunt.top/)，AI github search engine，自然语言搜索

## 权限控制

虽然个人账户的 Github SSH 密钥无法控制权限，所有 SSH 密钥都可以读写你的所有仓库。但是 Github 提供了另一个 Fine-grained PAT token，配合 gh cli 进行细粒度权限控制。

具体用法上网搜吧。反正这玩意可以控制访问哪些具体的仓库、读还是写权限、仓库内的各种资源（Release，Action 等），非常灵活。

拿到 token 后保存到临时文件，在 bash 里用 `gh auth login --with-token < token.txt` 即可。（任意 stdin 的输入都行）
...
- 另一个方法是用命令行下载([ref](https://www.bilibili.com/read/cv21459907))。例如使用 jq 和 aria2c：
  ```sh
...
访问 `https://github.com/<owner>/<repo>/compare/<old_commit>..<new_commit>.patch` 可以下载任意两个 commit 的 patch。
...
4. [GitHub Actions Has a Package Manager, and It Might Be the Worst](https://nesbitt.io/2025/12/06/github-actions-package-manager.html)，非常有道理的狂喷 Github Actions 的设计缺陷。
