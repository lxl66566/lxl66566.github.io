...
rust 的 mod 确实会让人摸不着头脑。建议先搜几篇文章看看，然后以下是我的一些浅薄理解，可能不正确，请自行分辨：
...
`cargo test` 默认不打印 _stdout_ 输出，想打印需要 `cargo test -- --show-output`。~~（rust 不能在 `Cargo.toml` 中直接指定 alias 我不是很认可）~~
