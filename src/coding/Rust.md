...
- 统一的代码格式、文档、测试、打包流程
  - 唯一指定顶级包管理器：cargo
- 静态检查给力，能过 $\approx$ 能跑，~~在 vibe coding 时代是非常可贵的能力~~
...
当然也有现成的库 [include_assets](https://github.com/hwjsnc/include_assets) 可以做到这一点。不过这个库基本不再维护，且 pull request 区也有指示其 bug，建议谨慎使用。
...
创建 `~/.cargo/config.toml`，参考[我的配置](https://github.com/lxl66566/nixos-config/blob/main/config/cargo.toml)。

然后由于现在大家都用 CI release，因此 `[profile.release]` 要写在项目里而不能写全局。

一些解释：

- [Book: Optimizing Build Performance](https://github.com/Kobzol/cargo/blob/performance-guide/src/doc/src/guide/build-performance.md)
- [checksum-freshness](https://doc.rust-lang.org/cargo/reference/unstable.html#checksum-freshness)：正常情况下 cargo 按 mtime 进行编译缓存，也就是即使不修改文件内容，只要按了保存，就要重新编译此 package 及所有依赖其的项。使用 checksum-freshness 可以根据文件 hash 进行编译缓存，也就是疯狂按保存而不修改内容是不会触发缓存失效的。
...
| [cargo-bloated](https://github.com/oxalica/cargo-bloated)| 用于优化产物体积。群友作品，据说比 cargo-bloat 更好用 |
...
| wasm-pack | vibe coding 重灾区，全是 emoji 但是关键内容缺失的大便一样的文档；还有[大便一样的代码质量](https://github.com/drager/wasm-pack/issues/1457)和放着根本不理的 issue 区 |
...
- 对于一般的小玩具，基本就只是支持一下颜色和时间戳输出。我之前一直在用 pretty-env-logger (based on env-logger)，不过由于 env-logger 引入了 regex，这玩意对编译后二进制大小有较大影响，所以我也在寻找更符合需求的替代品。
- 而对于更大一点的玩具，需要更多功能的，flexi_logger 会更舒适一些。
...
（拳打柴油机，[脚踢大海洋](https://t.me/withabsolutex/1836)）

我其实用 sqlx 习惯了，感觉也不一定需要 ORM 辅助开发，SQL 本身就是一个相对完备的转译兼容层，sqlx 也有编译期 SQL syntax check。

不过如果有看起来不错的 ORM，我也有一些意愿尝试。我个人还是倾向于基于 proc macro 的 ORM，结构即描述，看着比较干净一些。目前的记录：

- [tideorm](https://github.com/mohamadzoh/tideorm)：看 example proc macro 看着是挺简洁的。
- [toasty](https://github.com/tokio-rs/toasty)：tokio 团队出品（这不是一个褒义词）。
