...
注意安装完的提示，可以直接一行命令添加 `*.sh` 的打开方式绑定。
...
Git 的配置是大小写不敏感的，官方文档和 cmd output 都经常大小写混用，不要在意。
...
1. 配置个人信息
   ...
   ```
   这里强烈建议将 email `your-email@example.com` 设为你的 Github 注册邮箱，使 Github 能够关联你的 commit 和帐号，方便数据统计等。
2. 配置代理：由于众所周知的原因，最好使用代理上 Github。请在 `<port>` 处填写你的本地代理端口：
   ...
   Host github.com   # "github.com" 匹配的是仓库的 remote host name, 可以通过 git remote -v 查看。
       ...
       # 在 linux 系统上 ProxyCommand 会使用系统 default shell 执行，注意不要搞太复杂的管道等，避免兼容问题
   ```
3. 其他全局设置
   ```sh
   ...
   git config --global diff.algorithm histogram    # 更改为更智能的 diff 算法，可以减少合并冲突概率。详见页面底 external 1.
   ...
   git config --global merge.conflictstyle zdiff3  # 更智能的 diff 展示（只负责展示，不负责冲突检测）
   git config --global feature.manyFiles true      # 性能优化：启用一组 git 预设，可以减小仓库大小、加速 checkout
   git config --global core.fsmonitor true         # 性能优化：通过向 OS 注册一个 daemon，加速 git status/add
   # 全局忽略
   printf "node_modules\n__pycache__\n*.exe\n*.o\n" > ~/.gitignore_g
   git config --global core.excludesfile ~/.gitignore_g
   ```
   - 一些配置参考自[取消转义](#取消转义)、[external 8.](#external)
...
- [gitui](https://github.com/extrawurst/gitui)：TUI git 工具，虽然我是命令党。
- [Commitizen](https://github.com/commitizen/cz-cli)：帮助写出规范的 commit message，但是如果你养成了习惯也不需要这个。

## 基础

- 在 windows git bash 中，`ctrl + insert` 复制，`shift + insert` 粘贴，跟普通的终端有些区别。
...
> 在 windows 下由于权限问题可能会出现无法删除的情况，此时请在 bash `rm -rf .git`
...
注释是 commit 级别的，一个 commit 只能添加一次注释（但是可以多行）。~~我初学时还想着为修改的每个文件添加不同注释~~
...
git push origin <branch> --force-with-lease # 官方建议使用此选项代替 -f，更加安全
```
...
一般有两种下载方式，一种是本地已经有 git 仓库并且设置了 remote，此时使用 pull/fetch；如果本地没有 git 仓库，可以使用 clone。

```sh
git pull origin <branch>                  # 拉取远程分支并把文件检出，相当于 fetch + checkout。
git fetch origin                          # 从远程拉取最新分支，但是不要修改本地文件
git clone <gitrepo>                       # 在当前目录下创建文件夹并克隆完整仓库
git clone <gitrepo> --depth 1             # 仅克隆最新的一个提交，减少大小
...
这个其实是 linux bash 的相关内容，跟 git 没啥关系。不过在 Windows 进行开发、第一次接触脚本的用户还是可以看看。

新建 `xxx.sh`，输入每行一个指令，双击运行即可，本质是 bash 脚本，使用 `git-bash.exe` 执行了。

- 脚本执行完成后将自动关闭窗口。若需使之不自动关闭，请添加 `exec /bin/bash` 指令至脚本末尾。

### 设置

在前面的章节中已经用到了 `git config`。git 的配置分为仓库配置与全局配置，两者都是类似 toml 格式的文件。`git config xxx` 本质就是添加/修改配置文件中的条目。
...
- 修改上次 commit 代码内容：`git commit --amend`。
- 仅修改 commit message：`git commit --amend --no-edit`。
- 还有比较常用的，回到上一个 commit 的状态，去除所有多余文件和改动：`git reset --hard HEAD && git clean -f -d`。前半句是恢复所有 tracked 的文件，后半句则是清理 untracked 的文件。
...
branch 会随着开发而移动；tag 不会自动移动，代表一个提交树上的“锚点”。

与 branch 有本地和远程之分，tag 也有本地与远程。

- 可以通过 `git push origin :refs/tags/TAGNAME` 删除远程的 tag。

#### Github release 和 tag 是啥关系

这两者不是一个概念；tag 是一个一般不动的标签，release 特指发布新版本；在实现上，每一个 Github release 会关联一个 tag，但是 tag 并不一定要有对应的 release。

Github 上，手动新建 release 会让你关联一个 tag 或者新建一个 tag。如果删除了一个 release 关联的 tag 并推送这个删除事件到远端，此时 release 信息会被保留，状态更改为 draft。
...
1. 开启 histogram，减少冲突概率。
2. 如果线性分支并且有 squash，使用 rebase，保持提交树易读性。如果提交比较混乱，使用 merge，只需要解决一次冲突。
...
git stash push -u                # 将当前的 untracked 文件也加入 stash
git stash push -m "xxx"          # 临时保存代码，带注释
git stash push --keep-index      # 将当前状态推到 stash，并且保留暂存区的内容
```
...
- 更新：git 默认会使用 `.git/config` 里的 url 去拉取 submodule。如果你修改了 `.gitmodules` 的内容，必须用 `git submodule sync --recursive` 将内容同步到 `.git/config` 后，再执行 `git submodule update ...`。
...
虽然 git 一般都通过 git push、git pull 进行同步，但是对于一次性打包代码 + 不要把 `.gitignore` 里的文件打包进来的场合，使用 git archive 是最简单的。`git archive HEAD -o uno.zip` 可以直接将所有代码文件打包为 zip，便于发送等。
...
#### [git-sync-backup](https://github.com/lxl66566/git-sync-backup)

我写的一个小玩具，用于将 git 仓库作为备份工具，在不同设备上可以自定义同步行为和本地路径。对于文件可以指定硬链接；对于文件夹就是整体复制。

### fetch 忽略分支

如果有些分支比较大，并且没有必要拉到本地（例如自动构建的 gh-pages 产物等），可以设置 fetch 时忽略这个分支。

```toml
# 编辑 .git/config，在相应 remote 里设置 fetch = ^refs/heads/your_branch
[remote "origin"]
url = git@github.com:lxl66566/lxl66566.github.io.git
fetch = +refs/heads/code:refs/remotes/origin/code
...
sparse-checkout 是 git 的一个 feature，本质上就是 checkout 白名单，在 checkout 时可以只改变工作区的某些目录。

sparse-checkout 有两种模式，默认的是 no-cone，可以使用跟 `.gitignore` 一样的语法进行比较复杂的白名单筛选目录，但是性能较低；而新的 cone 模式只通过前缀筛选目录，性能高，代价是损失了一些灵活性。

sparse-checkout 本质上是白名单。如果你的需求是黑名单，sparse-checkout 可能不太适合，因为黑名单只能工作在 no-cone 下，会影响性能。

```sh
git sparse-checkout init --cone           # cone 模式初始化
git sparse-checkout init --no-cone        # no-cone 模式初始化
git sparse-checkout set "/*" "!/docs"     # 设置筛选列表 (会覆盖之前设的内容) (例子为 no-cone 语法)
git sparse-checkout add docs/             # 为筛选列表添加元素
```
...
7. [How Core Git Developers Configure Git](https://blog.gitbutler.com/how-git-core-devs-configure-git/)，但是，在抄配置的时候一定要搞清楚每一项的含义！！！[坑](https://t.me/withabsolutex/2576)
8. [Improve Git monorepo performance with a file system monitor](https://github.blog/engineering/infrastructure/improve-git-monorepo-performance-with-a-file-system-monitor/)
