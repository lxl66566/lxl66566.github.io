...
颜色有四个基本颜色[^5]与黑色。除了数字牌以外的牌均称作功能牌。黑牌（黑色牌）指上表中不带有`每色x张`字样或不标注颜色的牌。
...
## 效果优先级
禁，炸弹 > 后悔药 > 寡妇 > 神手，夹子 > 决斗，缴械，抬杠，c4等一般功能牌 > 反弹，pass，跳转等防御功能牌 > 转移，加牌
## 开局规则

游戏建议采用积分制（当然你也可以不用）。每位胜利的玩家获得x分，x为胜利后当前场上的存活人数。复活赛的玩家若胜利则同样计分。

初始手牌数可以选用5,7,9等数字（根据玩家数决定）。当手牌数超过`初始手牌数*3`时，玩家将不允许从牌堆获得手牌[^2]。
...
::: tabs#card
@tab 正式规则
[AOE效果](#aoe效果)：由发起者开始，出一张数字牌使 C4 被向下传递`x`家，`x`为此数字牌的大小。然后跳转到接到 C4 的玩家的回合，重复该过程。若某位玩家无法/拒绝打出数字牌，则 C4 爆炸，使该玩家弃掉所有手牌并重新摸等于初始手牌数的牌，效果结束，并由**发起者的下家**开始新的回合[^1]。过程中的数字牌将替换`last`。

> 允许自爆。
@tab 精简规则
从发起人开始，打出一张数字牌使 C4 被向下传递 *数字点数* 家，接到 C4 的玩家重复该过程。若不打出数字牌，则爆炸，使此玩家弃掉所有手牌并重新摸初始牌，并由**发起者的下家**开始新的回合。
:::
...
::: tabs#card
@tab 正式规则
[自生效效果](#自生效效果效果)：跟你的另一张手牌**同时**打出，将此手牌视为你指定的任一颜色。
@tab 精简规则
覆盖在另一张牌上打出，视为修改为任意颜色。
:::
## 已弃用功能牌
...
## tricks
* *天选之子（+全功能牌），迷彩* 是无副作用的胜利方法，可以不用喊出 `UNO` 而出完全部手牌获胜，且不会为自身遗留效果。
* 如果你的手牌很好，你最好留个 *寡妇*。
* *转移* 虽然使用条件严格，但是收益很高，在手牌少的时候尤为如此。同时，*转移* 对自己不利的牌，是成本很低的不使用而失去牌的手段。