...
|游戏名|问题|
| :-: | :-: |
| CHAOS;HEAD NOAH | [游玩列表](#游玩列表)中有一些更加详细的问题描述：文件路径带了 `;`，找不到 dll；只加载 `lib/windows-i686` 下的 dll；特效垃圾，拖拽窗口会停止；画面卡顿；不接受滚轮下滑；文字与语音不同步，有的文字强行分了两次点击；退出 backlog 需要两次右键，退出 TIP 要很多很多右键；视频音量不可调；调整窗口大小画面无法实时变更 |
