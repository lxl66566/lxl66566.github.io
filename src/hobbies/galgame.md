...
| 装甲恶鬼村正 | backlog 打断语音；视频声音不可调；过渡太长无法跳过；backlog 是所有文本，过长了；鼠标的某操作序列触发 bug，存档界面反复出现消失，游戏不再接受任何输入，无法关闭；选择支滚轮无法打开 backlog；ctrl 跳过太慢，超速跳过是非连续的，并且文本不会进 backlog；有的音效不会自动停止；esc 是退出游戏，经常以为是返回误点，然后进度又没了 |
