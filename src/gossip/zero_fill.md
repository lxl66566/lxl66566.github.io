...
一个 [Unix bug](https://en.wikipedia.org/wiki/Hidden_file_and_hidden_directory) 让 `.` 开头的文件成为了隐藏文件 -> 隐藏文件成为标准，大量软件对隐藏文件作出特殊处理 -> ripgrep 默认不扫描隐藏文件 -> 调用 ripgrep 的脚本跳过扫描某个 `.` 开头的关键文件 -> 因为跳过扫描造成生产事故 -> 因为生产事故丢了工作（仮）
