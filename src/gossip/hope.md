...
当然这些都是通过自定义化可以解决的问题，但是我觉得这些东西可以变成 vim 的默认设置而不是自定义设置。

- visual 下按 `(`,`[` 等括号类可直接为选中区域添加闭合括号。
- 保留非 vim 的 Ctrl 组合键功能，例如复制粘贴等。
- 删除选中区域时不复制到剪贴板。（区分删除和剪切）
- 更改指令查找替换方式（太麻烦了），直接搜索高亮，然后选择其中的一些或全部，进行直观替换。（这要求一个新 mode）
