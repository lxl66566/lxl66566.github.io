...
3. **降频使用**。CPU 默认值 100% 负载时会跑到 95 度撞墙降频，我也不希望 CPU 长时间保持在高温度影响寿命。7950x 本身的性能足够我使用，因此我尝试降频使用。在 AMD Ryzen Master 里先进入高级页面，选到 _Profile1_，调成 _节能模式_ 105W，其他都默认就行，然后应用。就这样，我居然用 65% 功耗跑了 94% 性能（94% 鲁大师得分），满载温度从 95 度 降到了 76 度！满载风扇声也小了很多。
   ...
   - Curve Optimize 跑了两个小时，给我的 per core OC 调到 -26。然后我一重启发现果然又炸了，进系统蓝屏，BIOS 没法动，又只能扣电池。后来乱搞乱搞进了 BIOS 把 Curve Optimize 又调回去了，这才成功开机。**不要碰 AMD Ryzen Master 会变得不幸。**
   - 一说，AMD Ryzen Master 的设置是系统级的([ref](https://www.reddit.com/r/AMDHelp/comments/jiu5ol/are_ryzen_master_overclocks_permanentchange_bios/))，无法期望在 windows 限制功耗后 linux 上也达到限制功耗效果；但是有一些部分又是可观测可持久的，例如 Curve Optimize 完后可以在 bios 里观测到。我本人验证 ryzen master 的设置是硬件级的，换到 linux 后设置仍然生效，功耗被限制在 105W。
...
AMD Ryzen Master 实际上只是给 bios 提供了一个图形化的界面，它里面的设置 bios 里也都有。那么到底要不要用呢？如果你是小白，我推荐你用，比较无脑好上手；但是如果你希望完全掌控你的 PC，或者希望榨干最后一丝性能，选择 bios 调会更好。

我刚开始使用 Ryzen Master，但是 Ryzen Master 其实[有各种各样的问题](./fuckxxx.md#amd-fuck-you)，因此最后还是选择了 bios。

## 调参

不调参就失去了 PC 的灵魂。而且最近几代 AMD 的参数都非常不像话，免费的能效不要白不要。

我的参数：

1. Curve Optimize 是一定要拉的，最近 7000 和 9000 系的旗舰 AU 都可以拉到 -23~-26。
2. 开启 AMD Eco，限制功耗为 105W。一个比较好的最高能效点。
   - ~~开启手动 PBO，限制 PPT 为 140W 左右，另外两个选项就交给 Auto。~~ 只开 PBO PPT 比较原始，可能还有一些其他问题。
3. 防掉压 CPU LLC，调到 Level 3 或者更高。
   - Auto 其实不太够用，在我的家用设备上边挖矿边干点别的很容易就会瞬间高负载，电压下探把 CPU 干到锁频。经常跑 xmrig，然后一觉醒来发现 hashrate 被锁在日常均值的 60%。
