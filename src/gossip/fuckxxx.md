...
此页面虽然看着暴论已经够多了，但是仍有本人相当多的暴论散落于博客各处。请点击链接前往查看：

- [音游暴论](../hobbies/rhythm_games.md#暴论-为什么你不该打音游)
- [浏览器暴论](../articles/browser/assess.md)
- [各种尝鲜体验](./userexp.md)

另外我的[个人频道](https://t.me/withabsolutex)里还有更多暴论，欢迎加入，一起喷这狗屎般的世界——
...
- `std::fs::canonicalize`：**rust 史上极恶函数之一** ([issue](https://github.com/rust-lang/rust/issues/59117))。这个函数在 Windows 上的第三方 fs、RAMDisk 等地方用就会返回 error（[ex1](https://github.com/rust-lang/rustup/issues/682)），但是各种应用和库的开发者不知道这些，导致这个函数被广泛用于各种 rust 程序上，比较知名的例如 nushell, uutils/coreutils, ouch 等。
...
- vscode 有一个机制是如果在打开窗口的情况下关机，开机后启动 vscode 会恢复这些窗口。这个机制本身是好的，但是恢复窗口以后这些窗口的相对位置可能发生改变，可能倒序、乱序，这又是什么意思。。而且 win11 是不允许调整窗口顺序的。
  ...
  - 但是 vscode 的关闭不是真正的关闭。。如果关闭某个窗口后立即重启，重启后打开 vscode 还会带上这个窗口。
...
- 权限管理稀烂。朋友圈等。
  - 群聊群主除了踢人，啥都干不了。
- linux 下没有可用的微信客户端。
  - _为了保障你的账号安全，暂不支持使用网页版微信。你可以前往微信官网 <https://weixin.qq.com/> 下载客户端登录。_
...
- 网页端本身的视频流拉取速度非常慢，估计被砍过。我用 yt-dlp 下载，可以比网页端快 10 倍以上。
- 网页端的缓存策略很差，即使在弱网环境下也不会多缓存一些时长，导致大量卡顿，损害用户体验。
...
- ~~中文（CJK）内容检索垃圾。使用空格分词检索模式，不适合该语系的内容检索。~~ 20260327 TG 服务端更新，支持中文搜索（分词实现）
- （PC）：无法针对聊天内容调整字体大小。
...
- 想钱想疯了，大会员无处不在。最开始推广大会员是非侵入式的，现在则是侵入式的，例如需要大会员才能置顶超过 5 个会话，等等。
- 还我的 ExLOLI、Visual Novel Channel。
- 2025 年下旬，telegram 注册新帐号需要交钱，开通一周的大会员（对于中国 +86 号码则是 7$ 左右），名义上是为了 sms。谁家 sms 这个价格呢我请问了。

## 各种地图类软件有多难用

### 高德地图有多难用

虽然说经常被调侃为缺德地图，但是高德地图在国内确实是做的比较好的了。

- 2023-2024 左右遇到的一个最致命的恶性 bug 是无法登录，直接把我送去了难用的百度地图。高德地图很多功能是需要登录后使用的，然而当时在我的手机上无法登录，即使我已经对其屏蔽 root、关闭代理，更换各种网络环境，就是无法登录，非常气人。
  - 后来修了，2025 年在同一设备上已经可用了。

### 百度地图有多难用

- 这源于我一次用百度地图的打车的惨痛经历。我在一个没有合适上车点的地方打车，四周都是高架。然后：
  1. 定位错误，让司机多跑了好远
  2. 我想走到能上车的地方，结果发现修改出发点有距离限制，而且只能修改一次。。
  3. 司机问我在哪，居然没法拍照发给司机，聊天窗口没法发图片。然后跟司机掰扯半天，因为我自己也不知道在哪，周围又没有标志性物体。高德地图是可以发图片的。
...
然后再建议看下 [停止用 Windows 工作! —— kokic](https://zhuanlan.zhihu.com/p/2024527609388627701) 这篇文章，站在开发者角度的批评更是尖锐中肯，我举双手赞成。
...
- windows bug 一堆，历史遗留重，设计也有缺陷（
  - windows 的设计理念是**所有程序的运行库全部自带**。这导致了混乱与体积膨胀。（但是 Windows 的兼容性也没得说，也不全是坏事）
...
- 为什么微软总是喜欢逼着别人用自家的垃圾？
  - 大力推广 copilot，一堆 copilot 图标，添加 copilot 按键等。但是 copilot 的智力实在是太低了，能被国产的小模型爆杀。
  - 还有 bing 搜索也是，每次 edge 更新都会给我把 bing 重新设为默认搜索引擎。但是 bing 比 google 落后得实在是太多了。
  - Windows 一直在推他的脑残 windows hello，逼着用户设置 pin 码，但是本质上和本地账户的密码又没有区别。
    - 我想方设法在登录 windows 账户的电脑上绕过 pin 码登录，但是都失败了，不设置 pin 码，登录就开始要微软账户密码了。最后只能退回本地账户。
...
- 微软的配置同步做的稀烂，我已经遇到过不止一次 edge 配置同步爆炸、丢失部分或全部配置的情况了。如果云端和本地配置不一致，微软不会让你选择，而是自己使用某些不可见的神秘机制进行合并处理。
- 微软自己的 mimalloc 和自己的 Windows 24H2 会打架，所有使用 mimalloc 预编译的软件都无法在 Windows 24H2 的某些特定版本上运行，并且非常难调试 ([src](https://t.me/withabsolutex/2730))

### WSL 有多难用

不可否认 WSL 确实有用，但是同时 WSL 也是一坨垃圾屎山，专门恶心开发者。
...
然而，这些驱动本就该是完美无缺的，对于任意负载、所有主流图形 API、任意的窗口交叠和快速移动、任意的窗口切换都应当能够保持正确而稳定的渲染。这些是图形驱动的基本功。
...
- 先说安装吧，_AMD Software: Adrenalin Edition_ 会携带**大量流氓傻逼组件**，例如 crash defender service 等，上述的 bug 就是这些没屁用的组件造成的。
  - Adrenalin Edition 还含有大量的推广内容，操作也很难受。
  - 即使我只需要一个最多数十 MB 的 driver，AMD 的下崽器也会把超过 1G 的全量软件拉到本地。
  - 安装时即便选择 `Driver only`，这些流氓组件也会被安装。
- 再说卸载，AMD 的这堆东西根本无法手动卸载干净，只能使用 AMD Cleanup Utility 进安全模式卸载。
  - 一个脑残 bug 是，我的 TMP/TEMP 环境变量指向 RAMDisk，进入安全模式后 RAMDisk 没法自动挂上，这两个环境变量不可达，然后 AMD Cleanup Utility 就打不开了，没有任何提示和报错，就是打不开。
    - 然后此时如果重启，则会继续进入安全模式，无法恢复正常使用。
  - 和傻逼微软的 edge 打出一套组合技：卸载完成后，会自动打开一个 report.xml（默认用 edge 浏览器打开）。但是安全模式下 edge 所有数据和登录信息都会丢失；最重要的是，重启到正常模式后，edge 的这些数据不会恢复，需要全部从云端同步，或者如果没有数据同步就直接丢失。
...
- 交易垃圾。
  ...
  - 送礼，付款时显示订单异常，无法下单
  - 充值 30 起充，不能提现余额。
- 202x 年，steam 大改了它的 UI。然后新的 UI 启动会抢占焦点，右键托盘退出甚至需要等待一段时间，估计是在等上报什么东西，真的无语。
- 改名垃圾。
...
百度网盘实在是太流氓了，我不敢想象有这样一个正经的软件可以干出这种事情。

- [YunShellExtV164.dll 定期扫进程使 CPU 暴涨](https://ourl.co/101766)
- [YunShellExtV164.dll 会注入所有 webview 进程，并且劫持 TCP 连接](https://t.me/withabsolutex/2618?single)
...
- 首屏启动时无法缩小屏幕。强制广告大屏播放，而且在公司开游戏太显眼了。
  - 启动到 94% 以后会有一段时间，模拟器窗口置顶，等于是强制播放广告。
