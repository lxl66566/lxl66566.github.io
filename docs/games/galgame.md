...
3. 确实，让我完全满意的程序少之又少，至今仍没有达到10分的程序出现（星光咖啡馆与死神之蝶，最高得分：9.9）
...
|冥契的牧神节|<text style="color:red;">存档&删除存档时出现过序号错乱，错误覆盖（仅发生一次）；<br/>auto时按左键停止后会继续向下跳一页</text>
...
还有，**一般（早年）作品都不可选择`Ctrl的快进是已读还是全部`**，由于是普遍性问题，不写进表中。（点名表扬`鯨神のティアスティラ`,`星光咖啡馆与死神之蝶`）
